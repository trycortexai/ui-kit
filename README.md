...
TypeScript 5.5's [isolated declarations](https://devblogs.microsoft.com/typescript/announcing-typescript-5-5-beta/#isolated-declarations) changes how declaration files are generated. Instead of analyzing your entire project to figure out types (slow), it processes each file independently (fast). The trade-off is that you must explicitly type your public exports - TypeScript won't infer them for you.

This transforms builds from seconds/minutes to milliseconds (**50-100x faster**), creates clearer APIs, and ensures compatibility with modern build tools.
...
// Not allowed: Inferred return type
export function getData() {
  return fetchUser(); // TypeScript would normally infer this
}
```

Learn more about isolated declarations [here](https://arshadyaseen.com/writing/isolated-declarations).

**For new projects, we strongly recommend isolated declarations for instant builds and clearer APIs.**

You might need to disable isolated declarations if you have complex generic types that are difficult to type explicitly (like advanced Zod schemas). Check the [Infer Types](#infer-types) section for this alternative approach.
...
::: tip
For new projects, stick with [isolated declarations](#isolated-declarations) (default behavior) for instant builds and clearer APIs. Only use `inferTypes` when explicit typing becomes impractical.
:::
